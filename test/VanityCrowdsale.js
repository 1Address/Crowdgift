...
contract('VanityCrowdsale', function ([_, ownerWallet, wallet, wallet1, wallet2, wallet3, wallet4]) {
    ...
    makeIt('should not be able to change owner wallet to 0 by owner', async function () {
        await crowdsale.setOwnerWallet(0).should.be.rejectedWith(EVMRevert);
    })

    makeIt('should not be able to change owner wallet by others', async function () {
        await crowdsale.setOwnerWallet(wallet1, { from: wallet1 }).should.be.rejectedWith(EVMRevert);
        await crowdsale.setOwnerWallet(wallet1, { from: wallet2 }).should.be.rejectedWith(EVMRevert);
        (await crowdsale.ownerWallet.call()).should.be.equal(ownerWallet);
    })
    ...
    makeSuite('before startTime', async function () {
        ...
        it('should fail to register', async function () {
            await crowdsale.registerParticipant({ from: wallet1 }).should.be.rejectedWith(EVMRevert);
        })
    ...
    makeSuite('before endTime', async function () {
        ...
        it('should not be able to finalize by anyone', async function () {
            await crowdsale.finalize({ from: wallet1 }).should.be.rejectedWith(EVMRevert);
            await crowdsale.finalize().should.be.rejectedWith(EVMRevert);
        })
    ...
    makeSuite('after endTime', async function () {
        ...
        it('should not be able to register anymore', async function () {
            await crowdsale.registerParticipant({ from: wallet2 }).should.be.rejectedWith(EVMRevert);
            await crowdsale.registerParticipant({ from: wallet3 }).should.be.rejectedWith(EVMRevert);
        })

        it('should not be able to finalize not by owner', async function () {
            await crowdsale.finalize({ from: wallet2 }).should.be.rejectedWith(EVMRevert);
            await crowdsale.finalize({ from: wallet3 }).should.be.rejectedWith(EVMRevert);
        })
        ...
        it('should not be able to distribute by anyone', async function () {
            ...
            await token.distribute(participantsCount, { from: wallet2 }).should.be.rejectedWith(EVMRevert);
            await token.distribute(participantsCount, { from: wallet3 }).should.be.rejectedWith(EVMRevert);
        })

        it('should not be able to distribute before finalization', async function () {
            const participantsCount = await crowdsale.participantsCount.call();
            await token.distribute(participantsCount).should.be.rejectedWith(EVMRevert);
        })
    ...
    makeSuite('after finalization', async function () {
        ...
        it('should fail to register', async function () {
            await crowdsale.registerParticipant({ from: wallet4 }).should.be.rejectedWith(EVMRevert);
        })
        ...
        it('should not be able to distribute not by owner', async function () {
            const participantsCount = await crowdsale.participantsCount.call();
            await token.distribute(participantsCount, { from: wallet1 }).should.be.rejectedWith(EVMRevert);
            await token.distribute(participantsCount, { from: wallet2 }).should.be.rejectedWith(EVMRevert);
        })

        it('should not be able to distribute for 0 or too many participants', async function () {
            const participantsCount = await crowdsale.participantsCount.call();
            await token.distribute(0).should.be.rejectedWith(EVMRevert);
            await token.distribute(participantsCount + 1).should.be.rejectedWith(EVMRevert);

            await token.distribute(participantsCount);

            await token.distribute(1).should.be.rejectedWith(EVMRevert);
        })
